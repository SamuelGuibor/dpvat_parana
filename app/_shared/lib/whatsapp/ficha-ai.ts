...
// Mesmo subset editável da ficha manual (client-info.ts) — email fica de fora
// de propósito: é unique no User e um palpite errado da IA quebraria login.
const AI_FIELDS = [
  "name", "cpf", "rg", "data_nasc", "data_acidente",
  "estado_civil", "profissao", "nome_mae", "cidade", "estado",
  "rua", "bairro", "numero", "cep", "hospital", "lesoes",
] as const;
...
const FIELD_LABELS: Record<AiField, string> = {
  ...
  data_acidente: "Data do acidente (DD/MM/AAAA)",
  ...
  cep: "CEP",
  hospital: "Hospital do atendimento",
...
Regras:
- Só preencha um campo se o dado estiver EXPLÍCITO na conversa ou legível num documento anexado. NUNCA invente, deduza ou complete parcialmente.
...
- Datas no formato DD/MM/AAAA. CPF/CEP com a máscara usual. Estado como UF (ex.: PR).
