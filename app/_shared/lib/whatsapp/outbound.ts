...
export async function findOrCreateContactByPhone(
  rawPhone: string,
  name?: string | null,
): Promise<{ id: string; phone: string; name: string | null; optedOut: boolean } | null> {
  ...
  if (rows.length) {
    return db.whatsAppContact.findUnique({
      where: { id: rows[0].id },
      select: { id: true, phone: true, name: true, optedOut: true },
    });
  }
  ...
  return { id: created.id, phone: created.phone, name: created.name, optedOut: created.optedOut };
}
...
export async function sendSystemWhatsApp(input: SystemSendInput): Promise<SystemSendResult> {
  try {
    ...
    if (windowOpen) {
      const result = await sendText(contact.phone, input.text);
      if (!result.waMessageId) {
        return { sent: false, via: "text", reason: result.error ?? "Meta rejeitou o envio" };
      }
      await persistSystemMessage(contact, result.waMessageId, input.text);
    ...
    await persistSystemMessage(contact, result.waMessageId, preview);
    await logWhatsAppEvent({
      action: "wa_template",
